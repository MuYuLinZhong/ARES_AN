...
import { Bell, LockOpen, Lock, Nfc, CheckCircle, XCircle } from 'lucide-react';
...
declare global {
  interface Window {
    AndroidBridge?: {
      onUnlockClicked(json: string): void;
      onLockClicked(json: string): void;
      onOperationCancelled(): void;
    };
    onLockProgress?: (payload: { progress: number; stepText: string }) => void;
    onLockOperationResult?: (payload: { success: boolean; message: string; isRetryable?: boolean }) => void;
  }
}

interface LockProgressState {
  active: boolean;
  progress: number;
  stepText: string;
  result: { success: boolean; message: string; isRetryable?: boolean } | null;
}

export default function Home({ onNavigate }: { onNavigate: (view: View) => void }) {
  const [lockState, setLockState] = useState<LockProgressState>({
    active: false, progress: 0, stepText: '', result: null,
  });

  useEffect(() => {
    window.onLockProgress = (payload) => {
      setLockState(s => ({
        ...s,
        active: true,
    ...
    window.onLockOperationResult = (payload) => {
      setLockState(s => ({
        ...s,
        progress: payload.success ? 100 : s.progress,
        stepText: payload.message,
        result: payload,
      }));
    };

    return () => {
      window.onLockProgress = undefined;
      window.onLockOperationResult = undefined;
    };
  }, []);

  const handleUnlock = useCallback(() => {
    setLockState({ active: true, progress: 0, stepText: 'Initiating...', result: null });
    window.AndroidBridge?.onUnlockClicked(JSON.stringify({ deviceId: '', deviceName: '' }));
  }, []);

  const handleLock = useCallback(() => {
    setLockState({ active: true, progress: 0, stepText: 'Initiating...', result: null });
    window.AndroidBridge?.onLockClicked(JSON.stringify({ deviceId: '', deviceName: '' }));
  }, []);

  const handleCancel = useCallback(() => {
    window.AndroidBridge?.onOperationCancelled();
    setLockState({ active: false, progress: 0, stepText: '', result: null });
  }, []);
    ...
    <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
      <header className="flex items-center justify-between px-6 pt-14 pb-4 z-10 shrink-0">
        ...
        <div className="flex flex-col items-center flex-1">
          ...
          <h2 className="text-lg font-extrabold tracking-tight">NFC Lock</h2>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 relative mb-24">
        <div className="relative flex flex-col items-center">
          <div className="relative flex items-center justify-center w-64 h-64 rounded-full border-4 border-primary/30 bg-gradient-to-tr from-primary/10 to-transparent shadow-[0_0_60px_10px_rgba(19,127,236,0.3)]">
            <div className="absolute inset-4 rounded-full border-2 border-dashed border-primary/40 animate-[spin_10s_linear_infinite]" />
          ...
          <div className="mt-12 text-center">
            ...
            <p className="mt-2 text-slate-400 text-sm font-medium">Tap a button, then hold phone near the lock</p>
          </div>
        </div>
      </main>
      ...
      <section className="px-6 pb-28 z-10 w-full shrink-0">
        <div className="flex gap-4 mb-4">
          ...
          <button
            onClick={handleLock}
            className="flex-1 h-16 flex items-center justify-center gap-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-lg transition-colors"
          >
            <Lock size={24} />
            Lock
          </button>
      ...
      {lockState.active && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 h-[55vh] bg-[#1a2533] rounded-t-[32px] z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.3)] flex flex-col items-center px-8 animate-in slide-in-from-bottom duration-300">
            <div className="w-12 h-1.5 bg-slate-700 rounded-full mt-4 mb-10" />
              ...
              {lockState.result ? (
                ...
                <>
                  ...
                  <h3 className="text-xl font-extrabold tracking-tight mb-2">
                    {lockState.result.success ? 'Success' : 'Failed'}
                  </h3>
                  <p className="text-slate-400 text-center font-medium max-w-[280px]">
                    {lockState.result.message}
                  </p>
                  <button
                  ...
                  <div className="relative w-32 h-32 flex items-center justify-center mb-8">
                    <svg className="w-full h-full -rotate-90">
                      ...
                      <circle
                        ...
                        style={{ strokeDasharray: 351.8, strokeDashoffset: 351.8 - (351.8 * lockState.progress) / 100 }}
                      />
                    </svg>
                    <div className="absolute flex flex-col items-center justify-center">
                      <span className="text-2xl font-extrabold text-primary">{lockState.progress}%</span>
                    </div>
                  </div>

                  <h3 className="text-xl font-extrabold tracking-tight mb-2">Processing...</h3>
                  <p className="text-slate-400 text-center font-medium max-w-[280px]">
                    {lockState.stepText || 'Please keep your phone close to the lock.'}
                  </p>
